...
public:
	...
	auto feedForward(const OldLayer& previousLayer) -> void;

	auto calculateOutputGradients(double targetVal) -> void;

	auto calculateHiddenGradients(const OldLayer& nextLayer) -> void;

	auto updateInputWeights(OldLayer& previousLayer) -> void;
...
private:
	...
	auto sumDOW(const OldLayer& nextLayer) const -> double;
